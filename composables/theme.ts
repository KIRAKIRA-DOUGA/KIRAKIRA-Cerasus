...
export const Theme = {
	...
	/** 设定主题模式（包含自动）。 */
	get themeSet() { return useState<ThemeSetType>("themeSet", () => "system"); },
	/** 主题色/调色板。 */
	get palette() { return useState<PaletteType>("palette", () => "pink"); },
	watchTheme() {
		watch(Theme.themeSet, themeSet => {
			if (themeSet === "system") autoChange();
			else Theme.theme.value = themeSet;
		});
		function autoChange() {
			if (Theme.themeSet.value !== "system") return;
		...
		// TODO: 如果设定值是深色，网页加载时会从默认值闪一下才变化过来。
	},
};
