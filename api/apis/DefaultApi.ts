...
import { HttpFile, HttpMethod, RequestContext, ResponseContext } from "../http/http";
import { ObjectSerializer } from "../models/ObjectSerializer";
import { canConsumeForm, isCodeInRange } from "../util";
import { BaseAPIRequestFactory, COLLECTION_FORMATS, RequiredError } from "./baseapi";
...
export class DefaultApiResponseProcessor {
	 ...
	 * @throws ApiException if the response code was not in [200, 299]
	 */
	public async users(response: ResponseContext): Promise<Videos200Response > {
		const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
		if (isCodeInRange("200", response.httpStatusCode)) {
