...
~下标~
```
上面两个功能来源于 [MultiMarkdown](https://rawgit.com/fletcher/MultiMarkdown-6-Syntax-Guide/master/index.html) 语法。

### 限制
* 停用对 HTML 语法的支持，一旦输入就会被转义为纯文本。警惕 <ruby>XSS<rt>小学生</rt></ruby> 攻击。
* 单下划线字符包裹文本将替换为下划线功能，取消原版下划线字符所表示的 `_倾斜_` _倾斜_ 和 `__加粗__` __加粗__ 等功能。
* 脚注功能在此处用途不是很大，可弃用。
...
### 转义
HTML 原生有对 Unicode 字符编码的支持，因此弃用以 `\u` 开头的语法。
```html
